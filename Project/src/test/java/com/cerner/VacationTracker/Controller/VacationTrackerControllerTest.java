...
//		mockVacationDetails2.setSerial_num("4");
	...
	void testGetassociateById() throws Exception{
		Mockito.when(vacationTrackerService.getAssociateById(Mockito.anyString())).thenReturn(Optional.of(mockAssociateDetails));
		MvcResult result = mvc.perform(MockMvcRequestBuilders.get("/api/VacationTracker/getProfileData/AS078223").accept(MediaType.APPLICATION_JSON)).andReturn();
	...
	void testCreateVacationDetails() throws Exception{
		...
		Mockito.when(vacationTrackerService.createVacationDetails(Mockito.any(Vacation.class))).thenReturn(mockVacationDetails);
	...
	void testCreateVacationDetailsException() throws Exception{
		assertEquals(vacationTrackerController.createVacationDetails(null).getStatusCode(), HttpStatus.INTERNAL_SERVER_ERROR);
	}
	...
	void testGetVacationHistoryByAssociateId() throws Exception {
		...
		Mockito.when(vacationTrackerService.getVacationHistoryByAssociateId(Mockito.anyString())).thenReturn(vacationList);
		MvcResult result = mvc.perform(MockMvcRequestBuilders.get("/api/VacationTracker/getAssociatevacationHistory/AS078223").accept(MediaType.APPLICATION_JSON)).andReturn();
	...
	void testGetVacationHistoryByAssociateIdException() throws Exception{
		assertEquals(vacationTrackerController.getVacationHistoryByAssociateId(null).getStatusCode(), HttpStatus.NO_CONTENT);
	}
	...
	void testGetVacationCountByAssociateId() throws Exception {
		...
		MvcResult result = mvc.perform(MockMvcRequestBuilders.get("/api/VacationTracker/getVacationCount/AS078223").accept(MediaType.APPLICATION_JSON)).andReturn();
